...
class ConsolidatedTreeTab(wx.Panel):
    ...
    def __init__(self, parent, project_panel):
        ...
        # TextCtrl ReadOnly com fonte monoespaçada e suporte a rich text
        ...
        self.HIGHLIGHT_BG_COLOR = wx.Colour(70, 70, 70) # Novo Fundo para destaque
        
        self.text_output.SetBackgroundColour(self.DEFAULT_BG_COLOR)
        self.text_output.SetForegroundColour(self.DEFAULT_FG_COLOR)
        
        self.highlight_range = (0, 0) # Rastreia o intervalo do destaque atual
    ...
    def select_path_in_tree(self, path: str, node_map: Dict[str, TreeNode]):
        ...
        # 1. Remove o destaque anterior (volta para a cor de fundo padrão)
        ...
        # Procuramos o nome do item seguido pela tag de tokens.
        # Procurar por uma substring no TextCtrl é mais estável que reconstruir o prefixo ASCII completo.
        # A busca pelo nome base + tag é única o suficiente.
        search_target = f"{node.name}{token_str}" 
        full_text = self.text_output.GetValue()
        
        # Encontra a posição do início da substring (nome+tokens)
        content_start_pos = full_text.find(node.name)
        
        if content_start_pos == -1:
            self.highlight_range = (0, 0) # Não achou
            return 
...
class SelectedFilesTab(wx.Panel):
    ...
    def __init__(self, parent, project_panel):
        ...
        self.sort_column = 2 # Padrão: Tokens
        self.sort_ascending = False # Padrão: Decrescente
        self.all_nodes_cache: List[TreeNode] = []

        # Binding para clique na coluna e DUPLO CLIQUE para Prévia (COMPORTAMENTO CORRETO)
    ...
    def on_item_activated(self, event):
        ...
        if idx >= 0 and idx < self.list_ctrl.GetItemCount():
            ...
            # Usa o método de prévia que orquestra o carregamento assíncrono
    ...
    def update_data(self, all_file_nodes: List[TreeNode], total_proj_tokens: int):
        """Atualiza a lista com base no filtro de busca (Sincronização)."""
    ...
    def _refresh_list(self):
        ...
        col_map = {
            ...
            1: lambda n: os.path.splitext(n.name)[1].lower(), 
            2: lambda n: n.token_count,                     
            3: lambda n: n.full_path.lower(),               
        }
        
        sort_key_func = col_map.get(self.sort_column)
        if sort_key_func:
            # Ordenação natural para Nome já está em col_map[0]
...
class ProjectPanel(wx.Panel):
    ...
    def _setup_bindings(self):
        ...
        # Árvore: Clique simples dispara o foco e o destaque na aba de Resumo da Árvore (Índice 0)
    ...
    # --- Lógica de Navegação da Árvore Lateral ---
    ...
    def on_tree_selection_changed(self, event):
        ...
        if item.IsOk():
            ...
            # 1. Muda o foco para a aba "Resumo da Árvore" (Índice 0)
            self.notebook.SetSelection(0) 
            
            # 2. Chama a função de destaque na aba de Resumo da Árvore
            if path in self.node_map:
                self.tab_tree.select_path_in_tree(path, self.node_map)

    # --- Lógica de Prévia (Assíncrona) ---
    ...
    def on_file_selected_for_preview(self, path: str):
        ...
        # 1. Vai para a aba de prévia (Índice 3)
        self.notebook.SetSelection(3) 
        
        # 2. Se não for arquivo de texto (ou seja, binário/ignorado), mostra o status.
        if not node.is_text:
            self.tab_prev.update_status_binary(path, node.size_bytes)
            return

        # 3. É arquivo de texto: Mostra status de carregamento
        self.tab_prev.update_status_loading(path, node.token_count)

        # 4. Inicia o carregamento real e processamento em segundo plano (Assíncrono)
        # O conteúdo já está em self.file_contents (lido durante o scan inicial)
    ...
    def _build_tree_recursive(self, parent_item, node):
        ...
        for child in sorted_children:
            ...
            if not child.is_dir and not child.is_text:
                self.tree_ctrl.SetItemText(new_item, f"{child.name} [IGNORADO]")
